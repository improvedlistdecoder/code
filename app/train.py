...
def test(model, device, test_loader, para,epoch):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            word_target = word_target.to(device)
            codeword_equal = (torch.sum(bool_equal, -1).to(device)
                              == word_target).to(torch.float)
            BER = 1 - (torch.sum(bool_equal) /
                       (results.shape[0] * results.shape[1]))
            FER = 1 - torch.sum(codeword_equal) / conf["para"]["test_batch_size"]
            FER_total.append(FER.cpu().numpy())
            BER_total.append(BER.cpu().numpy())
        FER = np.mean(FER_total)
        BER = np.mean(BER_total)
        snr = para["snr"]
        permu = para["num_permu"]
        logger.warning(f"Epoch={epoch},test_per={permu},SNR={snr},BER={BER:.7f},FER={FER:.7f}")
