...
class SignalDataset(Dataset):
    def __init__(self, conf):
        ...
        self.input_data = np.zeros([self.train_dataset_length, self.v_size + 1])
        self.label_data = np.zeros([self.train_dataset_length, self.v_size + 1])
        ...
        for i in range(self.train_dataset_length//self.batch_size):
            for snr in [1,2,3,4,5,6,7,8]:
                for j in range(20):
                    ...
                    self.label_data[m] = np.zeros(self.v_size + 1)
                    m = m + 1
    ...
    def awgn(self, snr):
        x = np.zeros(self.v_size + 1)
        x = 1 - 2 * x  # BPSK
        snr = 10 ** (snr / 10.0)
        xpower = np.sum(x ** 2) / (self.v_size + 1)
        npower = xpower / (2 * snr * self.rate)
        return x + np.random.randn(self.v_size + 1) * np.sqrt(npower), npower
...
class SignalTestset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        label1 = np.append(np.array([label1.sum()%2]), label1)
        modSignal = 1 - 2 * label1
        Es = 1
        snr = 10 ** (self.snr_vali / 10.0)
        npower = Es / (2 * snr * rate)  # npower = sigma2
        # N = np.zeros(n])a
        N = np.sqrt(npower) * np.random.randn(n + 1)
